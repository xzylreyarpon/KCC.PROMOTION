...
var reprintTranRow = null;
var tblTransactions = null;
var tblReprintTransactions = null;
...
//    getAllTransactions();
//    initEvents();
//    setInterval(keepSessionAlive,40000);
//}

function select_promotype(promotype) {
    revealModal("PopUpPromoType");
}


//function open_new_transaction(promotype) {
//   window.location.href = "SellingAreaCreate.aspx?promoType="+ promotype;
//}

function open_new_transaction() {
    window.location.href = "SellingAreaCreate.aspx";
}
...
function initEvents() {
    ...
    $("#btnIR").on("click", function () {
        ...
        revealModal('popupIncidentReport');
    })

    $("#btnSendIR").on("click", function () {
        var msg = $("#txtIRBody").val();
        if (msg == "" || msg == null) {
            notifyWarning("Please Write a Request Message");
        } else if (msg.length < 35) {
            notifyWarning("Message must be 35 characters and above");
        } else {
            if (confirm("Are You sure you want to Send this Request?")) {
                revealModal('loading');
                request_reprint(reprintTranRow.HEAD_ID, msg);
                hideModal('popupComposEmail');
                hideModal('popupIncidentReport');
            }
        }
    })
...
function initTblReprintTransEvents() {
    $("#tblReprintTransactions tbody").on('click', 'tr', function () {
        reprintTranRow = null;
        var tr = $(this).closest('tr');
        var row = tblReprintTransactions.row(tr);
        reprintTranRow = row.data();
        if (reprintTranRow != undefined) {
            if (reprintTranRow.STATUS == null || reprintTranRow.STATUS == 'null') {
                $('#txtIRBody').val('');
    ...
    $("#txtFilter_IR").on("keyup", function () {
        if (tblReprintTransactions != null) {
            //tblReprintTransactions.column(2).search($(this).val()).draw();
            tblReprintTransactions.search(this.value).draw();
        }
    })
}
...
function initTblTransactionEvents() {
    ...
    $("#tblTransactions tbody").on('click', 'tr', function () {
        ...
        if (transactionRow != undefined) {
            window.location.href = "SellingAreaCreate.aspx?headId=" + transactionRow.HEAD_ID;
        }
    });
...
function request_reprint(headId, msg) {
    msg = encodeURI(msg);
    var xmlhttp;
    ...
    xmlhttp.open("POST", "WebMethods/sellingAreaWebMethod.aspx/requestReprint", true);
    xmlhttp.responseType = "json"
    xmlhttp.setRequestHeader("Content-Type", "application/json; charset=utf-8");
    xmlhttp.send("{'headId':'" + headId + "','message':'" + msg + "'}");
}
...
function getAllTransactions() {
    ...
    xmlhttp.open("POST", "WebMethods/sellingAreaWebMethod.aspx/getAllTransactions", true);
    xmlhttp.responseType = "json"
    xmlhttp.setRequestHeader("Content-Type", "application/json; charset=utf-8");
    xmlhttp.send();
}
...
function getReprintTransactions() {
    ...
    xmlhttp.onreadystatechange = function () {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            response = jQuery.parseJSON(xmlhttp.response.d);
            if (tblReprintTransactions != null) {
                tblReprintTransactions.destroy();
            }

            tblReprintTransactions = $('#tblReprintTransactions').DataTable({
                "scrollY": "220px",
                "paging": false,
                "info": false,
                "filter": true,
            ...
            initTblReprintTransEvents();

        }
    }
    xmlhttp.open("POST", "WebMethods/sellingAreaWebMethod.aspx/getReprintTransactions", true);
    xmlhttp.responseType = "json"
    xmlhttp.setRequestHeader("Content-Type", "application/json; charset=utf-8");
    xmlhttp.send();
}
